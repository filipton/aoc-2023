fn main() {
    ...
    let pairs = get_all_pairs(&galaxies);
...
fn get_galaxies(map: &Vec<Vec<char>>, empty_space_factor: i128) -> Vec<(i128, i128)> {
    let mut galaxies: Vec<(i128, i128)> = vec![];

    let mut real_y = 0;
    for y in 0..map.len() {
        if map[y][0] == '@' {
            real_y += empty_space_factor;
            continue;
        }
...
fn expand_universe(input: &str) -> Vec<Vec<char>> {
    ...
    for line in input.lines() {
        ...
        if !line.chars().any(|c| c != '.') {
            // @ - special char that represents empty line
...
fn replace_column(lines: &mut Vec<Vec<char>>, column: usize, c: char) {
    for line in lines {
        line[column] = c;
    }
}

fn get_all_pairs<T>(input: &[T]) -> Vec<(&T, &T)> {
    let mut pairs = vec![];
